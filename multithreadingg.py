...
def extract_movies(soup):
        ...
        if ul is not None:
            ...
            for li in items:
                ...
                if a and '/title/tt' in a['href']:
                    movie_links.append('https://www.imdb.com' + a['href'])
        else:
            # Fallback: find any title links inside the container
            anchors = container.select("a[href*='/title/tt']")
            for a in anchors:
                href = a.get('href')
                if href:
                    movie_links.append('https://www.imdb.com' + href)
    else:
        # Fallback: search across the whole page
    ...
    threads = min(MAX_THREADS, len(unique_links))
    if threads == 0:
        print('Nenhum link de filme encontrado na página do IMDB.')
        return

    with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
        executor.map(extract_movie_details, unique_links)
